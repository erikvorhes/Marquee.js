...
$.fn.marqueeize = function(options) {
	...
	getBooleanString = function getBooleanString(booleanString) {
	...
	return this.each(function(){
		...
		if (marquee.hasClass("hasBeenMarqueed")) { 
			...
			marquee.each(function(){
				...
				marquee_instance.attr("data-original-length", total);
				// Used by infinite rotation
				if (marquee_instance.find(".counter .total").length > 0) {
					marquee_instance.find(".counter .total").html(total);
				} // Update the counter (1 of 6) if it exists
			});

			// Set WAI-ARIA roles & states:
			marquee
				.attr({"role": "tabs"})
				.find(".marquee-nav")
					.attr({"role": "tablist"})
					.find("li")
						.each(function(marqueeNavItemIndex) {
							var marqueeNavItem = $(this),
								marqueeIndex;
							marqueeNavItem.attr({
								"role": "tab",
								"tabindex": "0",
								"id": function() {
									if (!!marqueeNavItem[0].id) {
										return marqueeNavItem[0].id;
									}
									marqueeIndex = marqueeNavItem.closest(".marquee").index(".marquee");
									return "marquee_" + marqueeIndex + "-tab_" + marqueeNavItemIndex;
								}
							});
						})
						.find("a")
						...
						.end() // back to "li"
					.end() // back to .marquee-nav
				.end() // back to marquee
				.find(".marquee-viewport")
					.attr({"role": "presentation"})
					.find(".marquee-panels")
						.attr({"role": "presentation"})
						.find(".marquee-panel")
							.each(function(panelIndex) {
								var marqueePanel = $(this);
								marqueePanel.attr({
									"role": "tabpanel",
									"tabindex": "-1",
									"aria-hidden": function() {
										return getBooleanString(!marqueePanel.hasClass(settings.css_active_name));
									},
									"aria-labelledby": function() {
										return marqueePanel.closest(".marquee").find(".marquee-nav li:eq(" + panelIndex + ")")[0].id;
									}
								});
							})
						.end() // back to .marquee-panels
					.end() // back to .marquee-viewport
				.end(); // back to marquee
		}
	});
}
 ...
 *
 * 0.6.1
 * - Fixes a couple syntax errors, ensures functionality when jQuery.noConflict(); is used.
 *
 * 0.6
